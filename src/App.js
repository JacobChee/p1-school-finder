...
export default function App() {
  ...
  const filtered = useMemo(() => SCHOOLS.filter(s => {
    ...
    if (filters.search.trim() && !s.name.toLowerCase().includes(filters.search.trim().toLowerCase())) return false;
    if (homePos && s.lat && s.lng) {
      const dist = getDistanceKm(homePos[0], homePos[1], s.lat, s.lng);
      if (dist > radius) return false;
    }
    return true;
  }), [filters, homePos, radius]);
  ...
  const setChip = (key, val) => setFilters(prev => ({ ...prev, [key]: val }));
  ...
  return (
    <div className="app">
      ...
      <div className="sidebar">
        ...
        <div className={`school-list ${mobileView === "list" ? "mobile-show" : "mobile-hide-list"}`} ref={listRef}>
      ...
      {/* ── MAP PANEL ── */}
      <div className={`map-panel ${mobileView === "map" ? "mobile-show" : "mobile-hide"}`}>
