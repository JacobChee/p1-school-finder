...
export function LineChart({ data2c, data2b }) {
  if (!data2c) return null;
  const years = Object.keys(data2c);
  const vals2c = years.map(y => data2c[y]);
  const vals2b = years.map(y => data2b?.[y] || 0);
